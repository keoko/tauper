defmodule TauperWeb.GameLive.Component do
  ...
  def periodic_table(assigns) do
    ...
    <%=  for atomic_number <- Enum.concat(1..57, 72..88) do %>
        <div class={"element element-#{atomic_number} #{if @question != nil and  @question.atomic_number == atomic_number do 'selected' end}"}></div>
    <% end %>
    </div>
    """
  end
  ...
  def question_form(assigns) do
    ...
    <div>
        ...
        <div class="question-actions">
          <button type="button" phx-click="skip" class="button-secondary-outline"><%= gettext("Skip Question") %></button>
          <%= submit gettext("send answer"), phx_disable_with: gettext("Sending answer..."), class: "button-primary" %>
        </div>
        </.form>
    </div>
    """
